...
[<AbstractClass>]
type GenericUnitOfWorkService<'TUnitOfWork when 'TUnitOfWork :> IUnitOfWork and 'TUnitOfWork :> IDisposable and 'TUnitOfWork : (new : unit -> 'TUnitOfWork)>() = 
...
type UnitOfWorkService<'TContext when 'TContext :> DbContext>() =
    class
    inherit GenericUnitOfWorkService<UnitOfWork<'TContext>>()
    end